...
| -------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **[LINKEDIN_EXTENSION_CONTACT_URL_MATCH_FIX.md](./LINKEDIN_EXTENSION_CONTACT_URL_MATCH_FIX.md)**         | Extension: normalize LinkedIn contact URLs + slug fallback so older CRM contacts show View in CRM instead of Import |
| **[LINKEDIN_EXTENSION_DOM_EXTRACTION_UPDATE.md](./LINKEDIN_EXTENSION_DOM_EXTRACTION_UPDATE.md)**         | LinkedIn extension: removed AI/HTML extract API; DOM-only profile import via IDs/classes and `ProfileStructuredParser` |
...
| **[XTRAWRKX_USER_MANUAL.md](./XTRAWRKX_USER_MANUAL.md)**                                                 | Complete end-user manual: login credentials, Orbit (steps done), Fudge Base, Fudge Grow, Fudge Work, Landing CMS, Client Portal; Fudge Books excluded                      |
...
| **[CRM_COMPANY_PICKER_TYPEAHEAD.md](./CRM_COMPANY_PICKER_TYPEAHEAD.md)**                                   | CRM company dropdowns: API typeahead (`CompanyPicker`) instead of truncated first-page lists; Select menu clip fix |
...
| **[LANDING_SIGNUP_ONBOARDING_DATA_COLUMN_FIX.md](./LANDING_SIGNUP_ONBOARDING_DATA_COLUMN_FIX.md)**       | Fix landing signup failure: missing `client_accounts.onboarding_data` column on production Postgres (bootstrap + manual migration)                                              |
| **[LANDING_SIGNUP_ONBOARDING_EMAIL.md](./LANDING_SIGNUP_ONBOARDING_EMAIL.md)**                           | Onboarding confirmation email to personal address after new landing signup (portal link + next steps)                                                                         |
| **[LANDING_PASSWORD_RESET.md](./LANDING_PASSWORD_RESET.md)**                                             | Forgot password on landing + Client Portal: Firebase reset email (event SMTP sender), Strapi portal password sync                                                             |
...
| **[CLIENT_PORTAL_COMPANY_MEMBERS_UPDATE.md](./CLIENT_PORTAL_COMPANY_MEMBERS_UPDATE.md)**                 | Company Members page: CRUD via `/api/auth/company-members`, CRM contact sync linked to client account |
| **[CRM_CLIENT_PORTAL_PASSWORD_CHANGE.md](./CRM_CLIENT_PORTAL_PASSWORD_CHANGE.md)**                       | CRM client account Contacts tab: internal action to change a linked contact's client portal password |
| **[CLIENT_PORTAL_WORKFLOW_UPDATE.md](./CLIENT_PORTAL_WORKFLOW_UPDATE.md)**                               | Client portal hub: shared UI, task pipeline, client chat API, community CRM approval, XEN tiers, PM/CRM sync |
| **[MESSAGES_PAGE_CHAT_HUB_UPDATE.md](./MESSAGES_PAGE_CHAT_HUB_UPDATE.md)**                               | Portal Messages page: multi-location chat hub (All, Account, Tasks, Projects) aligned with CRM Client chat tab |
| **[CRM_CLIENT_ACCOUNT_UNIFIED_ACTIVITY_CHAT.md](./CRM_CLIENT_ACCOUNT_UNIFIED_ACTIVITY_CHAT.md)**         | CRM client account: single merged activity timeline + multi-location client chat hub (tasks, projects, deals) |
| **[TASK_STATUS_WORKFLOW_UPDATE.md](./TASK_STATUS_WORKFLOW_UPDATE.md)**                                   | Unified task status workflow: shared labels, steppers (client + internal), CRM/PM/portal alignment; Client workflow panel uses status stepper |
| **[CLIENT_PORTAL_PROJECTS_PM_ALIGNMENT.md](./CLIENT_PORTAL_PROJECTS_PM_ALIGNMENT.md)**                   | Client portal projects list + detail aligned with PM: table columns, sort, kanban, KPI cards, meta bar, tabs |
| **[CLIENT_PORTAL_COMMUNITIES_PM_ALIGNMENT.md](./CLIENT_PORTAL_COMMUNITIES_PM_ALIGNMENT.md)**             | Client portal communities list: PM-style tabs/toolbar, gallery + table views, live membership stats, no dummy KPIs |
| **[CLIENT_PORTAL_UI_ALIGNMENT.md](./CLIENT_PORTAL_UI_ALIGNMENT.md)**                                     | Client portal UI/UX aligned with CRM/PM: `PortalPageShell`, `KPICard`, brand orange, shared header/sidebar, modals, search |
| **[CLIENT_PORTAL_GLOBAL_SEARCH_UPDATE.md](./CLIENT_PORTAL_GLOBAL_SEARCH_UPDATE.md)**                     | Dashboard global search modal: tasks, projects, communities, company members via `WorkspaceSearchModal` |
| **[CLIENT_PORTAL_COMPLETE_PROFILE_UPDATE.md](./CLIENT_PORTAL_COMPLETE_PROFILE_UPDATE.md)**           | Client portal: Complete Profile page + header dropdown routing to `/profile` and `/settings` |
| **[CLIENT_PORTAL_HEADER_EXPORT_ONLY_UPDATE.md](./CLIENT_PORTAL_HEADER_EXPORT_ONLY_UPDATE.md)**       | Client portal headers updated to export-only: removed Add/Filter buttons, kept Export (CSV) aligned with CRM/PM |
| **[CLIENT_PORTAL_SUPPORT_TEAM_CARD_UPDATE.md](./CLIENT_PORTAL_SUPPORT_TEAM_CARD_UPDATE.md)**     | Client portal sidebar “Support Team” card: CRM-like orange gradient + assigned-POC UI state |
...
| **[LANDING_EVENT_REGISTRATION_DEADLINE.md](./LANDING_EVENT_REGISTRATION_DEADLINE.md)**                 | Landing events: admin registration deadline + auto-close public registration; enable/disable toggle on add/edit                                                              |
...
| **[CRM_DEAL_COLLABORATORS.md](./CRM_DEAL_COLLABORATORS.md)**                                             | CRM deals: multi-person `collaborators` on create/edit (PM-style assignee picker), notifications + delivery-project team seed |
...
| **[ACCOUNTS_ROLE_PERMISSION_DELEGATION.md](./ACCOUNTS_ROLE_PERMISSION_DELEGATION.md)**                   | Admin → Manager delegation: per-module permission assignment, partial role editing for Managers, `permissions.delegation` JSON                                           |
