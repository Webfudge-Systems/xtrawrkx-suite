      ...
      <div className="flex items-center -space-x-2">
        {visible.map((person) => (
          <Avatar
            ...
            title={person.name}
            className={`${AVATAR_RING} bg-gray-500 text-xs font-semibold text-white !h-8 !w-8`}
          />
        ))}
        {extra > 0 ? (
          <span
...
export default function ClientTaskDetailMetaBar({ task, className = "" }) {
  if (!task) return null;

  const priorityKey = (task.priority || "medium").toLowerCase();
  const flagClass = PRIORITY_FLAG_CLASS[priorityKey] || "fill-gray-400 text-gray-400";
  const priorityLabel = `${PRIORITY_LABELS[priorityKey] || "Medium"} Priority`;
  ...
  const projectName = task.project?.name || task.project || null;
