...
class BackgroundService {
    ...
    async showImportNotification(result, type) {
        ...
        try {
            ...
            console.warn('Could not show import notification:', error.message);
        }
    }
    ...
    async showErrorNotification(errorMessage) {
        ...
        try {
            ...
            console.warn('Could not show error notification:', error.message);
        }
    }
    ...
    handleTabUpdate(tabId, changeInfo, tab) {
        ...
        // Inject content script only on LinkedIn pages (only when scripting API is available)
        if (changeInfo.status === 'complete' && isLinkedIn && chrome.scripting) {
            chrome.scripting.executeScript({
                target: { tabId: tabId },
                files: ['src/content/linkedin-extractor.js']
            }).catch(() => {
                // Ignore — content script is already injected via manifest
            });
        }
