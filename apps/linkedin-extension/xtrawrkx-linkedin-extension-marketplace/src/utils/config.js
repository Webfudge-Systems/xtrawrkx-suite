    ...
    async getApiUrl() {
        try {
            ...
            if (stored.apiBaseUrl) {
                const storedUrl = stored.apiBaseUrl;
                const isLocalhost =
                    storedUrl.includes('localhost') || storedUrl.includes('127.0.0.1');

                // Auto-clear stale localhost URL when the extension is in production mode
                // (happens when a dev session leaves localhost stored in sync storage)
                if (isLocalhost && this.isProduction()) {
                    console.warn(
                        '[Config] Clearing stale localhost API URL — extension is in production mode.',
                        storedUrl,
                    );
                    await chrome.storage.sync.remove(['apiBaseUrl', 'environment']);
